...
- **MAP_BUPT_REAL**（北邮）、**MAP_BNU_REAL**（北师大）、**MAP_SCENIC_REAL**（天坛）为具体目的地的 OSM 真实内部道路图，前端显示 OSM 瓦片叠加路线
- **MAP_CAMPUS_OSM**（清华大学）和 **MAP_SCENIC_OSM**（颐和园）为 OSM 矢量模板，分别供其他学校和景区目的地复用
- **MAP_CAMPUS_001**、**MAP_SCENIC_001**、**MAP_MIXED_001** 为抽象内部地图模板（保留），前端不叠加真实瓦片
...
- **217 个目的地通过 internal_map_id 复用 8 套内部地图**（5 套 OSM 矢量 + 3 套抽象模板），避免为每个目的地单独制作地图
